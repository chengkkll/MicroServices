...
namespace TianCheng.Common
{
    ...
    public class AssemblyHelper
    {
            ...
            foreach (var library in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    ...
                    if (assembly.FullName.StartsWith("Microsoft") || assembly.FullName.StartsWith("System") || assembly.FullName.StartsWith("MongoDB.") ||
                        ...
                        assembly.FullName.StartsWith("Newtonsoft") || assembly.FullName.StartsWith("AutoMapper") || assembly.FullName.StartsWith("MongoDB.") ||
        ...
        static public IEnumerable<I> GetInstanceByInterface<I>()
        {
            ...
            List<Assembly> assemblyList = GetAssemblyList();
            foreach (var assembly in assemblyList)
            {
                foreach (var type in assembly.GetTypes())
                {
                    ...
                    {
                        // Todo : 是否应该修改为用DI的形式，不用本方法调用
                        yield return (I)Activator.CreateInstance(type);
                    }
                }
            }
        }
