...
namespace TianCheng.DAL.MongoDB
{
    ...
    public class MongoLog
    {
        ...
        static MongoLog()
        {
            ConnectionProvider connect = null;

            try
            {
                connect = ServiceLoader.GetService<ConnectionProvider>();
            ...
            catch
            {
                ...
                if (connect != null && !string.IsNullOrWhiteSpace(connect.MongoDefault.ConnectionString))
                {
                    conf.WriteTo.MongoDBCapped(connect.MongoDefault.ConnectionString, collectionName: LogCollectionName, restrictedToMinimumLevel: Serilog.Events.LogEventLevel.Error);
                }
