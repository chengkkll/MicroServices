...
namespace TianCheng.Organization.Services
{
    ...
    public class SystemMenuService : MongoBusinessService<MenuDAL, MenuInfo, MenuQuery>
    {
        ...
        {
            // 获取管理员账号的登录信息
            TokenLogonInfo adminLogonInfo = ServiceLoader.GetService<EmployeeService>().GetAdminLogonInfo();
            // 清除已有的菜单
            Dal.Drop();
            // 添加个人信息主菜单
            MenuInfo mainPersonal = new MenuInfo() { Name = "个人信息", Index = 51, Icon = "el-icon-eieco-personal" };
            await Create(mainPersonal, adminLogonInfo);
            await Create(new MenuInfo() { Name = "个人中心", Index = 1, Icon = "el-icon-menu", MainId = mainPersonal.IdString, Link = "PersonalCenter.Common" }, adminLogonInfo);
            await Create(new MenuInfo() { Name = "修改密码", Index = 2, Icon = "el-icon-menu", MainId = mainPersonal.IdString, Link = "PersonalCenter.ChangePwd" }, adminLogonInfo);
            // 添加系统管理主菜单
            MenuInfo mainSystem = new MenuInfo() { Name = "系统管理", Index = 52, Icon = "el-icon-eieco-system-manage" };
            await Create(mainSystem, adminLogonInfo);
            await Create(new MenuInfo() { Name = "部门管理", Index = 13, Icon = "el-icon-menu", MainId = mainSystem.IdString, Link = "System.Departments" }, adminLogonInfo);
            await Create(new MenuInfo() { Name = "员工管理", Index = 14, Icon = "el-icon-menu", MainId = mainSystem.IdString, Link = "System.Employees", }, adminLogonInfo);
            await Create(new MenuInfo() { Name = "角色管理", Index = 15, Icon = "el-icon-menu", MainId = mainSystem.IdString, Link = "System.Roles", }, adminLogonInfo);
            await Create(new MenuInfo() { Name = "菜单管理", Index = 19, Icon = "el-icon-menu", MainId = mainSystem.IdString, Link = "System.Menus", }, adminLogonInfo);
        }
        #endregion
        ...
        protected override void SavingCheck(MenuInfo info, TokenLogonInfo logonInfo)
        {
        ...
        /// <param name="logonInfo"></param>
        protected override void Updated(MenuInfo info, MenuInfo old, TokenLogonInfo logonInfo)
        {
            // 获取消息总线对象。
        ...
        /// <param name="info"></param>
        /// <param name="logonInfo"></param>
        protected override void Removing(MenuInfo info, TokenLogonInfo logonInfo)
        {
            // 删除菜单时。同时删除所有的下级菜单
